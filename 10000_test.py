...
def setup_cfg(weights_file):
    ...
    cfg.MODEL.ROI_HEADS.SCORE_THRESH_TEST = 0.05  # Lower threshold for more predictions
...
def generate_predictions_coco(predictor, dataset_dicts):
    ...
    with open("output/predictions_coco.json", "w") as f:
        json.dump(coco_results, f, cls=NumpyEncoder)

    print(f"Predictions saved in COCO format to output/predictions_coco.json")
...
def visualize_predictions(predictor, dataset_dicts, num_images=50):
    ...
    output_dir = os.path.join("output", "output_images")
...
def main():
    ...
    evaluator = COCOEvaluator("cesm_test", cfg, False, output_dir="./output/inference/")
